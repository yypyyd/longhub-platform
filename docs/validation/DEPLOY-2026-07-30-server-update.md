...
> 历史/废弃：这是旧产品上线/迁移记录，仅作审计存档；不代表当前已上线，也不作为 clean-launch
> Cloud API、Portal、Admin 或 Manager 的部署步骤。当前项目按全新环境重新部署，不迁移旧设备、订单或
> 数据；请以根目录 `README.md` 和 `EXECUTION_PLAN_V2.md` 为准。
